...
int main() {
	...
	createWindow(window, "Cubemap", 3, 2, windowWidth, windowHeight);
	if (!window)
		return false;
	...
	/* INIT SHADER */
	if (!cubeMap->initShader()) 
		return false;

	/* SETUP APPLICATION */
	cubeMap->setup_application();
	...
	while (!glfwWindowShouldClose(window)) 
	{
		...
		/* CAPTURE INPUT */
		processInput(window, cubeMap->getCameraObj(), deltaTime);
...
{
	cubeMap->getCameraObj()->ProcessMouseMovement(xoffset, yoffset);
}
...
{
	cubeMap->getCameraObj()->ProcessMouseScroll(yoffset);
}
